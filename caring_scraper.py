...
def MetaDescription(soup):
    ...
    return listToString([meta.attrs['content'] for meta in data if 'name' in meta.attrs and meta.attrs['name'] == 'description' ])

def communityName(soup):
    return soup.find(itemprop='name').h1.text.strip()

def getCommunityStreetAddress(soup):
    return soup.find(class_='street-address').text.strip()+ "," + getCommunityCity(soup) +"," + getCommunityState(soup) + getCommunityZipCode(soup)
